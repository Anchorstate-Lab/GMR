...
## The reader is in another crate, so this note is anchored at both ends

Setting the flag is half a mechanism. The other half is `budget.checkpoint()` in
`rescan`'s walk, once per file, in `batteries/survey` — one loop every extractor
goes through, so no probe has to remember to look ([[probe-budget]]).

The two are joined by a runtime convention, not by a type: nothing about
`invoke`'s signature says anyone reads what it sets, and `inproc.rs#invoke` does
not move when the reader does. A note about a mechanism spanning two crates has
to name both coordinates or it goes quiet exactly when one end is edited, which
is why `corpus.rs#rescan` is in `about:` above alongside [[probe-budget]]'s own
binding to it.
...
read the flag?** Deleting the checkpoint from `rescan` breaks nothing, compiles
