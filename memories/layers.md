...
这六个锚盯的是六个 crate 的公开面名单。`added` / `removed` 的意思不是
「有人写了个函数」，是**这一层对外承诺的东西变了**。
...
**这六个锚永远不会报 `sig` / `logic`。** roster 是表格形状，没有向量 ——
它只数名单，不看每一项的内容。内容归细颗粒度的锚，两层各管各的，
删掉任何一层另一层都补不上。

`added` 出现在 `gmr` 上 → 直接违反第 12 条，那一层只 re-export。
