...
- memories/：人写的记录。frontmatter 声明它关于哪个坐标，锚由此诞生 —— 这是本仓库**唯一**的锚来源
- .anchor/probes.toml：这些锚用的探针配方（名字 → 那个名字是什么）
- .codegraph/：CodeGraph 的本地索引

**没有 `.anchor/anchors.toml`。** 它是可选的：`gmr init` 不创建它，一个锚全部来自笔记的仓库根本没有这个文件。本仓库曾经有，因为旧笔记用裸键（`anchors: - doctrine::decisions`）只绑定、不声明，声明就得另找地方住。改成完整式 frontmatter（`- key/probe/position/shape`）之后声明和记忆住在同一个文件里，那份 toml 就没有存在理由了 —— 一个没有任何记忆认领的锚，`gmr doctor` 本来就会报成 barren。

机器读的配方住在 `.anchor/` 里，人写的记忆留在外面。这不是审美：toml 摊在仓库根目录会被 Agent 当成项目自身的代码读，而记忆藏进点目录就没人看得见 —— 两边都会毁掉这套东西的用处。`.anchor/` 里只有 `probes.toml` 跟 git，日志和制品不跟（见 `.anchor/.gitignore`）。
...
**代码里不写注释。** 一条都不写 —— 不是「尽量少写」，是零。要说的话写进锚定记忆，锚到那段代码的坐标上。

理由不是审美：注释和记忆各存一份，两份会分家，而且没有任何东西会发现。记忆有锚盯着，代码一动就报；注释没有。留一条注释就等于留一条不会被发现的漂移路径。
...
`crates/gmr-core` 已经清干净了（16 篇记忆代替了 50 行注释）。其余 76 处还在，按 crate 逐步搬。
