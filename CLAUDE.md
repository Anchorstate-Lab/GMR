...
- **`gmr-store`**: storage traits and feature‑gated backends, in the two tiers `gmr-content` has. Contracts every store must implement, sliced by **mutability**: `Journal` / `BindingStore` / `Sealer` / `LinkStore` / `Queue` / `Settings` / `Sightings`. Capabilities a store declines by not implementing them: `Chained`, `Usage`. Which tier a new trait joins is decided by one question — **can a store refuse it and still be a complete store?** A journal with no `Chained` is still a journal; a journal with no `append` is not.
