...
6. a domain can decide, from the resulting state, whether the memory should be handed back to a human or an agent.
...
### 3.7 Memory, reference, and binding

The runtime's core representation of a memory is a `Ref`: a provider identity and an external identifier. A `ContentProvider` can fetch the current content and, if supported, content at a requested historical version.

The core `Binding` relation connects a reference to one or more Anchor keys. It answers only “what is this memory about?” A `BindingRecord` adds the version observed when the relation was recorded and a binding-time sequence when one Anchor gives that sequence a unique meaning. Keeping these concepts separate allows rebinding to append a new occasion without changing the identity of the relation itself.
...
The runtime is assembled from four capability-focused services. `AnchorLog` wraps only the Journal. `Observer` holds only configured Transports. `MemoryLens` holds bindings, sealed blobs, links, and content providers. `Scheduler` holds the optional Queue, mutable run settings, and deployment policy. Operations use the smallest service set that can perform their work.
...
The coding domain wires together the substrate and batteries. It chooses the shell, in-process, and script transports, selects Git and Claude Code providers, uses survey for fuzzy coordinate matching, and links the language-aware extractor into the CLI. The substrate is therefore reusable by a different domain without importing a programming-language parser or a particular memory backend.
...
### 10.6 Operational and semantic failure
...
### 10.7 What is proved and what is assumed

The architecture proves several properties structurally. Journal replay is deterministic for a fixed sequence. Fact addresses distinguish derivation versions and absence. Successful and failed observation paths are separate. Terminal closure is monotonic in the projection. Stale fencing tokens cannot append, and neither can an unfenced sighting beside a lease-managed worker; the guard is a single shared function that every backend calls, so the in-memory reference store and SQLite enforce the same rule. Memory rewrite and Anchor staleness are exposed as distinct fields.
...
A worker that loses its lease may still finish its process invocation, but its fencing token is stale. The Journal rejects the append. This is the crucial property: lease expiry alone does not prove that an old worker stopped; the storage write must reject its late result.

An unfenced sighting cannot be written beside a lease-managed Anchor. A guarantee that holds only where the writers happen to be cooperative is not a guarantee, and a manual escape hatch is precisely where it would be lost, so the honest way to observe by hand is to take the lease. Author actions — revisions and closure — are exempt, because the guard covers sightings alone. They are not observations, do not compete as concurrent readings of the world, and would deadlock against a scheduler that is doing its job if they were.
...
Within that crate, a backend implements semantics rather than signatures. Journal and binding history must remain append-only, and the enforcement belongs in the store itself — a trait that merely declines to offer an update method is a convention, not a guarantee. Sealed records must be content addressed and immutable. The Journal must enforce fencing, including refusal of unfenced observations beside a lease-managed worker; the guard is shared rather than reimplemented, so a backend inherits the rule instead of restating it. Current binding queries must return the latest relation without deleting historical occasions. The backend must pass both the in-memory reference contract and the durable contract, and if it participates in export and import it must satisfy §9.5 as well.
...
Persistence interfaces, the SQLite implementation, and the portable export and import of §9.5 are in `crates/gmr-store`. The Journal, BindingStore, Sealer, LinkStore, Settings, and Queue are separate because their mutability and concurrency semantics differ. Runtime orchestration is in `crates/gmr-runtime`; `crates/gmr` provides the public re-export facade; the coding CLI performs declaration synchronization and memory delivery.
...
The coding domain will also read declarations from an optional `.anchor/anchors.toml`, but this repository deliberately has none and initialization does not create one. A bare key in that file binds without declaring, which puts the declaration in one place and the memory in another; when a note's frontmatter carries the whole declaration, the two live in the same file and the TOML has nothing left to say. Its remaining use is the narrow set of cases a coordinate cannot express — hand-written rules, a probe whose field of view is not the repository root, a position key with no coordinate syntax — and the domain's own linter reports a long-hand declaration that a coordinate could have routed. `.anchor/probes.toml` describes probe recipes and is tracked; observation state and artifacts under `.anchor/` are not. Neither file changes the substrate's ontology. `architecture.toml` configures the repository's dependency gate and is not part of GMR's runtime data.
