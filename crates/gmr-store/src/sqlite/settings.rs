...
impl Settings for SqliteQueue {
    async fn put(&self, anchor: &AnchorKey, settings: &RunSettings) -> Result<(), StoreError> {
        sqlx::query(
            "INSERT INTO settings (anchor, retain, cadence_secs) VALUES (?1, ?2, ?3)
             ON CONFLICT(anchor) DO UPDATE SET retain = ?2, cadence_secs = ?3",
        )
        .bind(anchor.as_str())
    ...
    async fn get(&self, anchor: &AnchorKey) -> Result<Option<RunSettings>, StoreError> {
        let row = sqlx::query("SELECT retain, cadence_secs FROM settings WHERE anchor = ?1")
            .bind(anchor.as_str())
