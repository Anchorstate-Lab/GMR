...
`check`, `observe` and `pass` hand memories back under `"memories"`, as `provider:external_id` — the address, not a display name. `git:memories/auth.md` is a path in this repository; `mem0:9f8e…` is a uuid in a mem0 scope. Use it verbatim with `gmr bind` / `gmr read`; do not strip the prefix, and do not assume a memory is a file.
...
- `skill_stale` — an installed copy of this doc differs from the one in the binary. `gmr init` only ever writes it when absent, so an upgrade leaves the old text in place and agents keep reading contracts this build no longer honours.
